    ...
    function fts_instagram_func($atts)
    {
        ...
        if (!isset($_GET['load_more_ajaxing'])) {
            ...
            if (isset($super_gallery) && $super_gallery == 'yes') { ?>
                <div class="fts-slicker-instagram masonry js-masonry <?php if (isset($popup) && $popup == 'yes') {
                    print 'popup-gallery ';
                }
                echo $fts_dynamic_class_name ?>" style="margin:auto" data-masonry-options='{ "isFitWidth": <?php if ($center_container == 'no') { ?>false<?php } else { ?>true<?php }
                if ($image_stack_animation == 'no') { ?>, "transitionDuration": 0<?php } ?> }'>
            <?php } elseif (isset($scrollMore) && $scrollMore == 'autoscroll' || isset($height) && $height !== '') { ?>
