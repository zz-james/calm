...
// 	cimy_switch_to_blog();
	...
	while ($i <= 2) {
		...
		$tiny_mce_objects = "";
		$notfirstTime=false;
		foreach ($fields as $thisField) {//THIS IS THE LOOP FOR EACH FIELD----------------------------------------------------------
			...
			// uploading a file is not supported when confirmation email is enabled (on MS is turned on by default yes) OH DEAR ------------------------------
			if (((is_multisite()) || ($options["confirm_email"])) && (in_array($type, $cimy_uef_file_types)))
				continue;
			...
			if (($i == 1) &&  (!$notfirstTime)){
			 	echo "<table id='form-table-cimy_wp_fields' class='form-table'>";
			} 
			...
			if (($fieldset > $current_fieldset) && (isset($fieldset_titles[$fieldset]))) { //&& ($i != 1)) {
				...
				if (isset($fieldset_titles[$current_fieldset])){
					...
					$fieldSetID = str_replace (" ", "", $fieldset_titles[$current_fieldset]);
					//echo "\n\t<h2>".$fieldset_titles[$current_fieldset]."</h2>\n<table id='form-table-".$fieldSetID."' class='form-table'>";
					echo "<table id='form-table-".$fieldSetID."' class='form-table'>";
				}
			...
			switch($type) {
				...
				case "dropdown-multi":
					...
					$obj_label = '<label for="'.$unique_id.'">'.$description.'</label>';
					$obj_class = ' class="'.$input_class.'"';
					$obj_type = '';
					$obj_value = '';
					$obj_value2 = $html;
				...
				case "checkbox":
					$obj_label = '<label class="cimy_uef_label_checkbox" for="'.$unique_id.'"> '.$label.'</label>';
			...
			if (($type != "radio") && ($type != "checkbox")) {//anything other than radio and checkbox
				echo '<tr id="'.$prefix.'p_field_'.$field_id.'"><th>';
				echo $obj_label;
				echo('</th><td>');
				}
				else {
				echo '<tr id="'.$prefix.'p_field_'.$field_id.'"><td colspan="2">';
				echo $obj_label;
				}
			...
			//if (!(($type != "radio") && ($type != "checkbox"))) {//radio and checkbox
			//	echo $obj_label;
			//}
				
			if (($description != "") && ($type != "registration-date") && ($type != "dropdown")) {
				echo "\t";
